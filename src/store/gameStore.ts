...
export const useGameStore = create<GameStore>((set, get) => ({
  ...
  nextMonth: () => {
    const state = get()
    if (state.currentEvent || state.currentEmergency) return // 有未处理事件时不推进
    ...
    for (const member of next.cabinet) {
      ...
      for (const [key, value] of Object.entries(member.bonuses)) {
        const metricKey = key as keyof typeof cabinetMetrics
    ...
    if (next.turn % TERM_LENGTH === 0) {
      // 暂停时间，进入大选阶段（保留 next 中已应用的内阁加成、二级指标更新等）
      set({
        ...next,
        gamePhase: 'election',
        timeSpeed: 0,
      ...
      return // 不继续后续事件触发，等待玩家在大选页面完成竞选
    }

    // 提前下台检查
    const early = checkEarlyEnd(next)
    if (early.ended && early.reason) {
      const grade = calcGrade(next.metrics)
      set({
        ...next,
        endingReason: early.reason,
        endingGrade: grade,
        screen: 'ending',
      })
      checkAchievements(set, get, { ...next, endingGrade: grade })
    ...
    if (emergency) {
      next.currentEmergency = emergency
      next.currentEvent = null
    } else {
      // 检查入侵事件
      const invasion = checkInvasion(next)
      if (invasion) {
        next.currentEmergency = invasion
        next.currentEvent = null
      } else {
        // 触发新事件
        next.currentEvent = pickEvent(next)
      }
    }

    // 随机触发质询（每6回合约1次，大幅降低频率）
    if (!next.currentDebate && !next.currentEmergency && next.turn % 6 === 0) {
      const debateChance = next.metrics.approval < 40 ? 0.25 : 0.1
    ...
    // 周期性生成选区信件（每8回合，降低频率）
    if (next.turn % 8 === 0 && next.pendingLetters.length < 3) {
    ...
    // 周期性生成外部照会（每12回合，降低频率）
    if (next.turn % 12 === 0 && next.pendingNotes.length < 2) {
    ...
    if (next.pmStats.partyPrestige < 30 && next.turn - (next.lastUltimatumTurn || 0) >= 8) {
      ...
      // 选择 consolidate / reform_promise 会通过 resolveOption 后续修改 partyPrestige
    }
    ...
    if (coalitionSeats < totalSeats * 0.5 && next.turn > 6) {
      // 触发不信任投票事件
  ...
  executeDiplomaticAction: (countryId, actionId) => {
    ...
    // 冷却：基于回合差
    if (state.turn - country.lastActionTurn < action.cooldown) return
